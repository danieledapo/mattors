...
export class IrregularIslands implements ISketch {
    public readonly name = "Irregular Islands";

    public readonly width = 600;
    public readonly height = 600;

    public readonly circleDeformations = 50;
    public readonly perturbations = 200;
    ...
    private drawDrop(p: p5) {
        ...
        for (let i = 0; i < this.perturbations; ++i) {
            this.drawPoly(p, this.irregularize(p, polylinePoints));
        }
    }
    ...
    private irregularize(p: p5, polylinePoints: p5.Vector[], minArea?: number): p5.Vector[] {
        const out: p5.Vector[] = [];
        ...
        for (let i = 0; i < polylinePoints.length; ++i) {
            ...
            if (minArea !== undefined && d < minArea) {
                continue;
            }
